...
import src.psy_style as pstyle
...
def process_session(bsid, session, complete=True,version=None,format_options={},refit=False):
    '''
    ...
    stimulus_presentations = pm.annotate_bouts_loc(session)
   
    print("Formating Data")
    format_options = get_format_options(version, format_options)
    psydata = format_session(session, stimulus_presentations, format_options)
...
def annotate_stimulus_presentations_np(stimulus_presentations,ignore_trial_errors=False):
    '''
...
def format_session(session,stimulus_presentations, format_options):
    '''
        Formats the data into the requirements of Psytrack
        ARGS:
            data outputed from SDK
            format_options, a dictionary with keys:
                timing, 0/1, if True (Default), then timing is a vector of 0s and 1s, 
                    otherwise, is -1/+1
    ...
    annotate_stimulus_presentations_np(stimulus_presentations,
