...
def annotate_licks(session):
    ...
    # stim_start=session.stimulus_presentations.query('~omitted')['start_time'].values[0]
    non_omitted = session.stimulus_presentations[~session.stimulus_presentations['omitted']]
    ...
    session.licks['pre_ili'] = np.concatenate([
        [bout_threshold*2],np.diff(session.licks.timestamps.values)])
    session.licks['post_ili'] = np.concatenate([
        np.diff(session.licks.timestamps.values),[bout_threshold*2]])
...
def annotate_bouts_loc(session, stimulus_presentations):
    ...
    for _, stim in stimulus_presentations.iterrows():
        ...
        stim_end = stim_start + stimulus_duration
        licks_during_stim = session.licks[(session.licks['timestamps'] >= stim_start) & 
                                          (session.licks['timestamps'] < stim_end)]
        stimulus_presentations.loc[_, 'licked'] = len(licks_during_stim) > 0
    ...
    stimulus_presentations['bout_number'] = np.nan

    for _, bout_start in bout_starts.iterrows():
        start_time = bout_start['timestamps']
        stim_index = stimulus_presentations[stimulus_presentations['start_time'] <= start_time].index[-1]
        stimulus_presentations.loc[stim_index, 'bout_start'] = True
        stimulus_presentations.loc[stim_index, 'num_bout_start'] += 1
        stimulus_presentations.loc[stim_index, 'bout_number'] = bout_start['bout_number']
    ...
    for _, bout_end in bout_ends.iterrows():
        end_time = bout_end['timestamps']
        stim_index = stimulus_presentations[stimulus_presentations['start_time'] <= end_time].index[-1]
        stimulus_presentations.loc[stim_index, 'bout_end'] = True
        stimulus_presentations.loc[stim_index, 'num_bout_end'] += 1
    ...
    #     "Number of bouts doesn't match between licks table and stimulus table"
    ...
    #     "All licking bout starts should have licks"
    ...
    # assert np.all(~(stimulus_presentations['in_lick_bout'] & \
    #     stimulus_presentations['bout_start'])), \
    #     "Can't be in a bout and a bout_start"

    return stimulus_presentations


def annotate_bouts_original(session):
    '''
        Uses the bout annotations in licks to annotate stimulus_presentations
    ...
    session.stimulus_presentations.loc[overlap_index,'in_lick_bout'] = False
